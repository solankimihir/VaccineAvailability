...
async function getVaccineAvailablity(districtID, dateForCheckingAvailability) {
  ...
  for (i = 0; i < data.sessions.length; i++) {
    ...
    if (data.sessions[i].available_capacity > 0) {
      ...
      if (
        document.getElementById('optionsAge18plus').checked &&
        data.sessions[i].min_age_limit < 45
      ) {
        if (
          document.getElementById('optionsDose1').checked &&
          document.getElementById('optionsDose2').checked
        ) {
          ...
          insertRowInTable(
            ...
            districtName,
            `18+ Any Dose`,
            data.sessions[i].name,
            data.sessions[i].pincode,
            data.sessions[i].available_capacity_dose1,
            data.sessions[i].available_capacity_dose2
          )
        } else if (
          document.getElementById('optionsDose1').checked &&
      ...
      if (
        document.getElementById('optionsAge45plus').checked &&
        data.sessions[i].min_age_limit > 18
      ) {
        if (
          document.getElementById('optionsDose1').checked &&
          document.getElementById('optionsDose2').checked
...
function insertRowInTable(
  ...
  _name,
  _pinCode,
  _available_capacity_dose1,
  _available_capacity_dose2
) {
